...
def Airflow_3_dubchak_tags_read_data_csv():
    @task()
    def get_data_dubchak():    
        df = requests.get(PATH, stream=True)
        zipfile = BytesIO(df.content)
        df_year = zipfile.read(FILE_DW).decode('utf-8')
        df_year = df.query("Year==@year") 
        return df_year

    @task()
    def get_top_global_s_game_dubchak(df_year):
        df_year_1 = pd.read_csv(df_year)
        global_s_game = df_year_1.groupby('Name',as_index=False)['Global_Sales'].sum()
    ...
    def get_top_genre_EU_dubchak(df_year):
        df_year_1 = pd.read_csv(df_year)
        eu_s_genre = df_year_1.groupby('Genre',as_index=False)['EU_Sales'].sum()
    ...
    def get_top_platform_NA_dubchak(df_year):
        df_year_1 = pd.read_csv(df_year)
        game_Platform_NA_best =df_year_1.query("NA_Sales>1").groupby('Platform',as_index=False)['Name'].count()
    ...
    def get_top_publisher_JP_dubchak(df_year):
        df_year_1 = pd.read_csv(df_year)
        publisher_mean_jp = df_year_1.groupby('Publisher',as_index=False)['JP_Sales'].mean()
    ...
    def get_best_game_EU_JP_dubchak(df_year):
        df_year_1 = pd.read_csv(df_year)
        game_salse_EU_JP = df_year_1.groupby('Name',as_index=False)[['EU_Sales','JP_Sales']].sum()
...
Airflow_3_dubchak_tags_read_data_csv = Airflow_3_dubchak_tags_read_data_csv()
