...
def print_data(ds):
    ...
    print(f'Top 10 domain zones for date {ds}: {top_10}')
    print(f'Longest domain name for date {ds}: {longest_name}')
    print(f'Airflow.com rank for date {ds}: {airflow_rank}')
...
dag_andrew_stalin = DAG('lesson2_andrew_stalin', default_args=default_args, schedule_interval=schedule_interval)

t1 = PythonOperator(task_id='get_data',
                    python_callable=get_data,
                    dag=dag_andrew_stalin)
