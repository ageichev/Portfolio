...
def m_strojkin_3():
    ...
    def get_top_name(data):
        global_top = data.groupby('Name', as_index=False)                         .Global_Sales.sum()                         .sort_values('Global_Sales', ascending=False)                         .iloc[0,0]
        return global_top
    ...
    def get_na_platform(data):
        top_platform_NA = data.loc[data.NA_Sales > 1]                                     .groupby('Platform', as_index=False)                                     .agg({'Name': 'count'})                                     .sort_values('Name', ascending=False)                                     .rename(columns={'Name': 'Number'})
