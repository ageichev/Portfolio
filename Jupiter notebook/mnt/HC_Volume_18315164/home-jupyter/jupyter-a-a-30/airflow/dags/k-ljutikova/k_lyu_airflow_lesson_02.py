...
def get_top10():
    ...
    top10_df = df.domain.apply(lambda x: x.split('.')[-1]).value_counts().head(10).index.to_list()
    top10_df = ", ".join(top10_df)
...
def get_longest():
    ...
    longest = df\
        .loc[df.domain.apply(lambda x: len(x)==max_length)]\
        .sort_values('domain')\
        .head(1)\
        .reset_index()\
        .domain[0]
...
dag = DAG('k_lyu_lesson2', default_args=default_args, schedule_interval=schedule_interval)
