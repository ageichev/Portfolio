...
def get_top_10_dzones(): # топ-10 доменных зон по численности доменов
...
def get_longest_domain(): # домен с самым длинным именем 
    top_data_df = pd.read_csv(TOP_1M_DOMAINS_FILE, names=['rank', 'domain'])
...
def get_airflow_rank(): # место домена airflow.com
    top_data_df = pd.read_csv(TOP_1M_DOMAINS_FILE, names=['rank', 'domain'])
...
dag = DAG('lesson_2_i_borovikova', default_args=default_args)
