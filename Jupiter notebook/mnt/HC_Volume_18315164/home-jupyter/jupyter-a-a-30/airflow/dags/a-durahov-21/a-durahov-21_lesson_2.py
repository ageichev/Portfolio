...
def get_max_name():
    ...
    domain_max = top_data_df['domain'][top_data_df.domain.apply(lambda x: len(x.split('.')[0])==domain_max_length)].sort_values().iloc[0]
    with open('domain_max.txt', 'w') as f:
        f.write(domain_max)
...
def print_data(ds):
    ...
    with open('domain_max.txt', 'r') as f:
...
dag = DAG('a_durahov', default_args=default_args, schedule_interval=schedule_interval)
