...
def d_semenov_test_TG():
    ...
    def get_World_top_1_game(data):
        World_top_1_game = data.head(1)['Name']
    ...
    def print_data(World_top_1_game, TOP_EU_Genre, Platform_1M, TOP_1_Publisher_JP, Number_EU_better_than_JP): # передаем переменные с прошлых функций
    ...
    @task()
    def send_to_TG(World_top_1_game, TOP_EU_Genre, Platform_1M, TOP_1_Publisher_JP, Number_EU_better_than_JP):
        context = get_current_context()
        date = context['ds']
        dag_id = context['dag'].dag_id
        year = 1994 + hash('d-semenov') % 23
        message = f'''Well Done! Dag {dag_id} completed on {date}. It's fine.
                      The most popular game in {year} year for {date}')
                      {World_top_1_game}
                      
                      The most popular genre(s) in {year} year for {date}
                      {TOP_EU_Genre}

                      The most popular platform(s) with more than 1M copies in NA in {year} year for {date}
                      {Platform_1M}

                      The most popular Publisher(s) with best sales in Japan in {year} year for {date}
                      {TOP_1_Publisher_JP}

                      Number of games with better sales in Europe than in Japan in {year} year for {date}
                      {Number_EU_better_than_JP}'''
        bot = telegram.Bot(token=BOT_TOKEN)
        bot.send_message(chat_id=CHAT_ID, text=message)
    ...
    send_to_TG(World_top_1_game, TOP_EU_Genre, Platform_1M, TOP_1_Publisher_JP, Number_EU_better_than_JP)

d_semenov_test_TG = d_semenov_test_TG()