...
def get_stat():
    ...
    top_data_top_10 = top_data_df[top_data_df['domain']].count()
...
def get_stat_com():
    top_data_df = pd.read_csv(TOP_1M_DOMAINS_FILE, names=['rank', 'domain'])
    top_data_top_10 = len(top_data_df[top_data_df['domain']]).sort_values(ascending=False)
    top_data_top_10 = top_data_top_10.head(1)
    with open('top_data_top_10_com.csv', 'w') as f:
        f.write(top_data_top_10.to_csv(index=False, header=False))
        
def get_stat():
    top_data_df = pd.read_csv(TOP_1M_DOMAINS_FILE, names=['rank', 'domain'])
    top_data_top_10 = top_data_df[top_data_df['domain']=='airflow.com']
    with open('top_data_top_10.csv', 'w') as f:
        f.write(top_data_top_10.to_csv(index=False, header=False))
...
dag = DAG('top_10_lys', default_args=default_args, schedule_interval=schedule_interval)
