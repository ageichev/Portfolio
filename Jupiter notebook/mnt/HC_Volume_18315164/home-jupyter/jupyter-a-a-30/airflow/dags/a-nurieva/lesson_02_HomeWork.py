...
def get_top_domain_zone():
    ...
    top_data_df['domain_zone'] = top_data_df.domain.apply(lambda x: x[x.rfind(".")+1:])
...
def get_domain_length():
    ...
    max_domain = top_data_df.query(f'domain_length=={max_length}')\
            .sort_values('domain', ascending=True).head(1)
    with open('max_domain.csv', 'w') as f:
        f.write(max_domain.to_csv())

def get_airflow()
    top_data_df = pd.read_csv(TOP_1M_DOMAINS_FILE, names=['ranks', 'domain'])
    top_data_df['domain']=top_data_df['domain'].astype(str)
    get_airflow = top_data_df.query('domain=="airflow.com"')
     with open('get_airflow.csv', 'w') as f:
        f.write(get_airflow.to_csv())   

        
def print_data(ds):
    with open('data_top_10.csv', 'r') as f:
        data_top_10 = f.read()
    with open('max_domainm.csv', 'r') as f:
        max_domain = f.read()
    with open('get_airflow.csv', 'r') as f:
        get_airflow = f.read()        
    date = ds
    ...
    print(f'The longest domain name for date {date} is {max_domain.domain.values[0]}')

    if len(get_airflow.ranks.values) == 0:
    print(f'There is no domain airflow.com for date {date}')
    else:
    print(f'For date {date} the rank of domain airflow.com:')
    for i in get_airflow.ranks.values:
        print(i, end = ' ')
...
dag = DAG('domain_info_nurieva', default_args=default_args, schedule_interval=schedule_interval)
