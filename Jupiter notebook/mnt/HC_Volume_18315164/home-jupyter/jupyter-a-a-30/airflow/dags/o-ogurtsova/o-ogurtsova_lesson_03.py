...
def top_games_o_ogurtsova():
    ...
    def get_best_selling_genre_Europe(df_my_year):
        ...
        best_selling_genre_Europe = df_my_year            .groupby('Genre', as_index=False)            .agg({'EU_Sales' : 'sum'})            .query('EU_Sales == @max_EU_sales')            .reset_index(drop=True)            ['Genre']            .tolist()
    ...
    def get_best_platform_million_copies_NA(df_my_year):
        ...
        best_platform_million_copies_NA = df_my_year.query('NA_Sales > 1')            .groupby('Platform', as_index = False)            .agg({'Name' : 'count'})            .query('Name == @max_NA_names')            .reset_index(drop=True)            ['Platform']            .tolist()
    ...
    def get_best_avg_sales_publisher_Japan(df_my_year):
        ...
        best_avg_sales_publisher_Japan = df_my_year.groupby('Publisher', as_index = False)            .agg({'JP_Sales' : 'mean'})            .query('JP_Sales == @max_JP_avg_sales')            .reset_index(drop=True)            ['Publisher']            .tolist()                                                      
...
top_games_o_ogurtsova = top_games_o_ogurtsova()
