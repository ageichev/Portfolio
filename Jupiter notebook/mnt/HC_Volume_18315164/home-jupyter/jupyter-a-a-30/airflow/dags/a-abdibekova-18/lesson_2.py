...
def get_max_length():
    ...
    max_length_domain=top_data_domain.sort_values(by=['length_of_name'],ascending=False).head(1)
    with open('max_length_domain.txt', 'w') as f:
        f.write(str(max_length_domain))
...
def print_data(ds):
    with open('top_domain_10.csv', 'r') as f:
        all_data = f.read()
    with open('max_length_domain.txt', 'r') as f:
        all_data_len = f.read()
...
dag = DAG('a-abdibekova', default_args=default_args, schedule_interval=schedule_interval)
...
t4 = PythonOperator(task_id='get_max_length',
