...
def top_ru_com_3_lesson():

    @task(retries=3)
    def get_data(): # при использовании декораторов id функции - это id таска
    ...
    def get_table_ru(top_data):
        ...
        # даг ожидает, что на выходе будет объект, который может быть переведен в json, поэтому немного видоизменяем табличку
    ...
    def print_data(ru_stat, com_stat):
        ...
        ru_avg, ru_median = int(ru_stat['ru_avg']), int(ru_stat['ru_median'])
        com_avg, com_median = int(com_stat['com_avg']), int(com_stat['com_median'])

        print(f'''Data from .RU for {date}
                  Avg rank: {ru_avg}
                  Madian rank: {ru_median}''')

        print(f'''Data from .COM for {date}
                  Avg rank: {com_avg}
                  Madian rank: {com_median}''')
...
top_ru_com_3_lesson = top_ru_com_3_lesson()