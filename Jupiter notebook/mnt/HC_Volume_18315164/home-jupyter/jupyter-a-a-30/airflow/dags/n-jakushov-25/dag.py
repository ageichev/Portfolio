...
def get_zones():
    ...
    with open('top_10_zones_njakushov.csv', 'w') as f:
...
def max_len_zone():
    ...
    with open('max_len_zone_njakushov.csv', 'w') as f:
        f.write(zone)
...
def get_airflow_rank():
    ...
    with open('airflow_rank_njakushov.csv', 'w') as f:
        f.write(airflow_rank.to_csv(index=False, header=False))


def print_data(ds):
    with open('top_10_zones_njakushov.csv', 'r') as f:
        top_10_zones = f.read()
    with open('max_len_zone_njakushov.csv', 'r') as f:
        max_len_zone = f.read()
    with open('airflow_rank_njakushov.csv', 'r') as f:
        airflow_rank = f.read()
    date = ds
...
dag = DAG('n-jakushov-25', default_args=default_args, schedule_interval=schedule_interval)

t1 = PythonOperator(task_id='get_data_njakushov25',
                    python_callable=get_data,
                    dag=dag)

t2 = PythonOperator(task_id='get_zones_njakushov25',
                    python_callable=get_zones,
                    dag=dag)

t3 = PythonOperator(task_id='max_len_zone_njakushov25',
                    python_callable=max_len_zone,
                    dag=dag)

t4 = PythonOperator(task_id='get_airflow_rank_njakushov25',
                    python_callable=get_airflow_rank,
                    dag=dag)

t5 = PythonOperator(task_id='print_data_njakushov25',
