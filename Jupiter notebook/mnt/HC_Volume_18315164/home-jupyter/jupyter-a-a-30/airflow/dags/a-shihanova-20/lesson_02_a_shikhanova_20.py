...
def get_name_domain():
    top_data_df = pd.read_csv(TOP_1M_DOMAINS_FILE, names=['rank', 'domain'])
    top_data_df['length_name'] = top_data_df.domain.apply(lambda x: len(x))
    longest_domain=top_data_df.sort_values('length_name', ascending = False).head(1)
    with open('name_domain.csv', 'w') as f:
        f.write(str(longest_domain.domain).split()[1])
