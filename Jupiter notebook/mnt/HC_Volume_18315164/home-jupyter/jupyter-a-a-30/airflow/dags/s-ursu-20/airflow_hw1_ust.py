...
def get_airflow_rank():
    ...
    if len(top_data_df.query('domain == "airflow.com"')) > 0:
        ans = f'''Airflow is on {top_data_df.query('domain == "airflow.com"')['rank'].to_list()[-1]} position'''
    else:
...
dag_ursu = DAG('domain_stats_ursu', default_args=default_args, schedule_interval=schedule_interval)
