...
def top_10_domain_zone(): # Top 10 domain zones by domain's quantity
    ...
    top_10_domain_zone_df = (top_doms_df['domain']
                          .apply(lambda x: x.split('.')[-1])
                          .value_counts()
                          .to_frame().reset_index().head(10))
...
def max_length_domain(): # Domain with the longest name (the first one in alphabetical order if several)
    ...
    max_length_domain_str = (top_doms_df
                                  .sort_values(['domain_len', 'domain'], 
...
dag = DAG('maks-morozov_lesson2', default_args=default_args, schedule_interval = schedule_interval)
